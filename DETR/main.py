...
def main(args):
    ...
    if args.resume:
        ...
        if not args.eval:
            test_stats, coco_evaluator, detections = evaluate(
                model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir
            )
    
    if args.eval:
        test_stats, coco_evaluator, detections = evaluate(model, criterion, postprocessors,
    ...
    #         with (tb_log_dir / "log.txt").open("a") as f:
    ...
    #     metric_names = ["AP50_95", "AP50", "AP75", "AP_small", "AP_medium", "AR100"]
    #     for idx, name in enumerate(metric_names):
    #         value = float(coco_stats[idx])  # numpy scalar 转 float
    ...
    # 初始化 TensorBoard writer，只在主进程初始化
    if utils.is_main_process():
        ...
        # 选择写入的关键指标
    key_metrics = [
        "loss", "loss_bbox", "loss_ce", "loss_giou", 
        "class_error_unscaled", "cardinality_error_unscaled"
    ]
    
    # 训练过程
    for epoch in tqdm(range(args.start_epoch, args.epochs + args.start_epoch), desc="Epoch"):
        if args.distributed:
            sampler_train.set_epoch(epoch)
        ...
        # TensorBoard：记录 train 和 test 的所有指标（只在主进程中写入）
        if utils.is_main_process() and writer is not None:
            for k in key_metrics:
                if k in test_stats:
                    writer.add_scalar(f"test/{k}", test_stats[k], epoch)

        # 记录 COCO evaluator 指标（只在主进程中写入）
        coco_stats = coco_evaluator.coco_eval["bbox"].stats
        metric_names = ["AP50_95", "AP50", "AP75", "AP_small", "AP_medium", "AR100"]
        if utils.is_main_process() and writer is not None:
            for idx, name in enumerate(metric_names):
