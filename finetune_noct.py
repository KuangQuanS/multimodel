...
def load_encoder(mod, args):
    ...
    encoder = ModalityEncoder(dim_in=dim_in, dim_latent=args.latent_dim)
...
def train(args):
    ...
    # Load encoders
    encoders = {mod: load_encoder(mod, args).to(args.device) for mod in args.modalities}
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(y)), y), 1):
        ...
        for epoch in range(1, args.epochs+1):
            ...
            if val_f1 > best_f1:
                best_f1 = val_f1
                torch.save(model.state_dict(), os.path.join(args.output_dir, f"fold{fold_idx}_best.pth"))
            
            if epoch % 10 == 0:
                print(f"Epoch {epoch}: Train Acc={train_acc:.1f}%, Val Acc={val_acc:.1f}%, F1={val_f1:.4f}, AUC={val_auc:.4f}")
        
        fold_results.append({'f1': best_f1, 'auc': val_auc})
